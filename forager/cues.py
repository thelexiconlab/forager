...
def create_semantic_matrix(path_to_embeddings):
    ...
    semantic_matrix_df.to_csv(pkg_path + '/../data/lexical_data/USE_semantic_matrix.csv', header=False, index=False)
...
class phonology_funcs:
    ...
    def create_phonological_matrix(labels):
        ...
        pkg_path = os.path.dirname(os.path.abspath(__file__))
        phon_matrix_df.to_csv(pkg_path + '/../data/lexical_data/USE_phon_matrix.csv', header=False, index=False)
        return sim
